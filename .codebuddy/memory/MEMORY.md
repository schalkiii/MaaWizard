...
- **前端结构**：`App.vue` 三标签页(运行/图编辑器/录制)。**设备连接面板(ControllerPanel)已合并进运行页**，不再单独设设备 tab（之前分两页被判定为重复）。运行页含：加载动态库/资源、ControllerPanel(连接窗口或ADB)、运行时状态(每1.5s自动刷新 runtimeStatus)、入口节点下拉+运行/停止、『查看窗口画面』(控制器截图，仅截取连接窗口区域)、识别回显(绿色识别框)。运行时后端把识别结果（节点名/是否命中/识别框[4]/命中风截图）随 `maa://event` 回传。图编辑器三层：`graph.ts`(纯数据映射层)、`PipelineNodeView.vue`、`JumpBackNodeView.vue`、`GraphEditor.vue`(网格背景/缩放/缩略图+拖拽连线)；`RoiCapture` 框选抓取后直接显示模板缩略图（用 scrap 桌面截屏，全屏，便于任意框选）；`NodeInspector`、`RecorderPanel`、`help/registry.ts`。**AI 页已从导航移除**（原 AiPanel 是调起 uvx maafw-cli 的开发者调试台，非终端用户功能，代码保留可恢复）。
...
- **Tauri 本地图片显示（asset 协议）**：前端用 `convertFileSrc(path)` 把本地路径转成 `asset://localhost/...` 后，必须在 `tauri.conf.json` 中显式开启 `app.security.assetProtocol.enable = true` 并配置 `scope`（如 `{"requireLiteralLeadingDot": false, "allow": ["**/*"]}`），同时在 `Cargo.toml` 的 `tauri` 依赖中加上 `features = ["protocol-asset"]`，否则 `<img>` 会显示 broken image。
- **运行失败排查**：点击「运行」前后端会先 `load_resource(resourceDir)` 自动加载对应资源包；`run_task_blocking` 以 `MaaStatus::SUCCEEDED` 判断成败（之前 `job.wait()` 只 wait 不判断，会误导性报「执行完成」却伴随 `Tasker.Task.Failed` 事件）。入口节点名必须存在于已加载资源包中。`list_resources` 只把含 `pipeline/` 或 `image/` 子目录的目录视为资源包，排除 `resource/pipeline` 这类内部目录，下拉不再出现 resource/pipeline。
- **两种截图分工（不重复）**：运行页『查看窗口画面』= `controller_screenshot`，走已连控制器，只截取目标窗口区域；图编辑器/录制页 `RoiCapture`『截取屏幕』= `capture_desktop`（scrap），截整屏用于任意框选 ROI。两者目的不同，保留各自。
